...
def prepare_tiku(tokenizer, parallel):
    data = json.load(open('/mnt/ceph2/datasets/tiku/mix/conversations_tiku_ocr_1M.json', encoding='utf-8'))
    progress_queue = multiprocessing.Queue()
    if parallel:
        process_parallel(data, tokenizer, progress_queue, '/mnt/ceph2/datasets/tiku/opt/conversations_tiku_ocr_1M.json')
    else:
...
def check():
    import cv2
    conversations = json.load(open('/mnt/ceph2/datasets/tiku/opt/conversations_tiku_ocr_1M.json', encoding='utf-8'))
