...
DEFAULT_CKPT_PATH = '/mnt/ceph2/Vary/runs/04222/'
...
def _launch_demo(args, model, tokenizer, image_processor, image_processor_high):
    ...
    def get_ocr(data, w, h):
        ...
        for item in data['regions']:
            ...
            if item['result'][0] == '':
                continue
            region_3point = item['region_3point']
            ...
            regions.append({'bbox': [x0, y0, x1, y1], 'result': item['result'][0]})
        regions = sorted(regions, key=lambda x: x['bbox'][3])
        ocr = ''
        for region in regions:
            ocr += f'{str(region["bbox"])}:{region["result"]}\n'
        return ocr
    ...
    def predict(_chatbot, image):
        ...
        payload = {}
        files = [('image', ('tmp.jpg', open('tmp.jpg', 'rb'), 'image/jpeg'))]
        headers = {}
        response = requests.request("POST", url, headers=headers, data=payload, files=files)
        data = json.loads(response.text)
        json.dump(data, open('tmp.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=4)
