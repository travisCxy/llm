...
def prepare_tiku(tokenizer, parallel):
    ...
    if parallel:
        process_parallel(files, tokenizer, False, True, progress_queue, '/mnt/ceph2/datasets/tiku/vary/conversations_tiku_det3.json')
    else:
...
def prepare_sjb(tokenizer, parallel):
    ...
    progress_queue = multiprocessing.Queue()
    if parallel:
        process_parallel(files, tokenizer, True, False, progress_queue, '/mnt/ceph2/datasets/tiku/vary/conversations_sjb_det3.json')
    else:
...
def check():
    docx = Docx()
    conversations = json.load(open('/mnt/ceph2/datasets/tiku/vary/conversations_sjb_det3.json', encoding='utf-8'))
    print(len(conversations))
