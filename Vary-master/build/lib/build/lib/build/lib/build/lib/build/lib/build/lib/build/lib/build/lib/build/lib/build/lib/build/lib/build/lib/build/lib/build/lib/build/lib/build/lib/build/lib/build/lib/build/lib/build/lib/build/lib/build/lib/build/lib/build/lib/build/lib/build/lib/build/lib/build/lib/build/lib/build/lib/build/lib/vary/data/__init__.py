...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances):
        ...
        if 'loc_inputs' not in instances[0]:
            return dict(
                input_ids=input_ids,
                labels=labels,
        ...
        loc_inputs = [x['loc_inputs'] for x in instances]
        loc_targets = [x['loc_targets'] for x in instances]
        loc_inputs = torch.tensor([list(a) for b in loc_inputs for a in b])
        loc_targets = torch.tensor([list(a) for b in loc_targets for a in b])
...
def make_supervised_data_module(interleave, with_box, tokenizer, data_args):
    ...
    elif data_args.conversation_version == 'llama':
        from vary.data.conversation_dataset_llama import ConversationDataset
        dataset_cls = ConversationDataset
