...
def prepare_tiku(tokenizer, parallel):
    files0 = glob(f'/mnt/ceph/tiku/images0/**/*.jpg', recursive=True)
    files0 += glob(f'/mnt/ceph/tiku/images1/**/*.jpg', recursive=True)
    files0 += glob(f'/mnt/ceph/tiku/images2/**/*.jpg', recursive=True)
    ...
    if parallel:
        process_parallel(files, tokenizer, False, progress_queue, '/mnt/ceph/tiku/mix/conversations_tiku_ocr_1M.json')
    else:
...
def check():
    ...
    import cv2
    docx = Docx()
    conversations = json.load(open('/mnt/ceph/tiku/mix/conversations_tiku_ocr_1M.json', encoding='utf-8'))
