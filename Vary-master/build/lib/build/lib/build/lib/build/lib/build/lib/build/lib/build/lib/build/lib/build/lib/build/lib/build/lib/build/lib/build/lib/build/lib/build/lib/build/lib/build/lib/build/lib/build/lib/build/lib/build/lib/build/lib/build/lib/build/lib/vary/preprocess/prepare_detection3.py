...
def prepare_tiku(tokenizer, parallel):
    ...
    #             count[1] += 1
    #             if '<w:tblGrid>' not in xml_content:
    #                 count[2] += 1
    # print(count)

    progress_queue = multiprocessing.Queue()
    if parallel:
        process_parallel(files, tokenizer, False, True, progress_queue, '/mnt/ceph2/datasets/tiku/vary/conversations_tiku_det6.json')
    else:
...
def prepare_sjb(tokenizer, parallel):
    ...
    progress_queue = multiprocessing.Queue()
    if parallel:
        process_parallel(files, tokenizer, True, False, progress_queue, '/mnt/ceph2/datasets/tiku/vary/conversations_sjb_det6.json')
    else:
...
def check():
    import zipfile
    docx = Docx()
    conversations = json.load(open('/mnt/ceph2/datasets/tiku/vary/conversations_sjb_det6.json', encoding='utf-8'))
    print(len(conversations))
    for c in tqdm(conversations):
        path = '/mnt/ceph2/datasets/tiku/' + c['image']
        if '200890647-1_crop_ppn_1213' not in path:
            continue
        # if 'split' not in path:
        #     continue
        # skip = True
...
if __name__ == "__main__":
    ...
    # conversations = json.load(open('/mnt/ceph2/datasets/tiku/vary/conversations_tiku_det5.json', encoding='utf-8'))
    # counts = {}
    # for c in tqdm(conversations):
    #     subject = c['image'].split('_')[-2]
    #     if subject not in counts:
    #         counts[subject] = {'count': 0, 'split': [0, 0], 'pic': [0, 0], 'table': [0, 0], 'cols': [0, 0]}
    #     counts[subject]['count'] += 1
    #     counts[subject]['split'][0 if 'split' in c['image'] else 1] += 1
    #     text = c['conversations'][1]['value']
    #     counts[subject]['pic'][0 if 'pic' in text else 1] += 1
    #     counts[subject]['table'][0 if 'table' in text else 1] += 1
    #     if 'images0-2' in c['image']:
    #         name = os.path.basename(c['image'])[:-4]
    #         counts[subject]['cols'][0 if name[-1] == '2' else 1] += 1
    # for subject in counts:
    #     print(subject, counts[subject])
    # print('count:', sum([counts[subject]['count'] for subject in counts]))
    # print('split:', sum([counts[subject]['split'][0] for subject in counts]))
    # print('pic:', sum([counts[subject]['pic'][0] for subject in counts]))
    # print('table:', sum([counts[subject]['table'][0] for subject in counts]))
    # print('cols:', sum([counts[subject]['cols'][0] for subject in counts]))

    # docx = Docx()
    # conversations = json.load(open('/mnt/ceph2/datasets/tiku/mix/conversations_tiku_1M.json', encoding='utf-8'))
    # print(len(conversations))
    # for c in tqdm(conversations):
    #     text = c['conversations'][1]['value']
    #     path = '/mnt/ceph2/datasets/tiku/' + c['image']
