...
class varyQwenModel(QWenModel):
    ...
    def encode_input_embeds(self, input_ids, images, loc_embeds, inputs_embeds=None):
        ...
        box_token = 151853
        if (input_ids == box_token).sum() > 0 and loc_embeds is not None:
            inputs_embeds[input_ids == box_token] = loc_embeds.type(inputs_embeds.dtype)
...
class varyQwenForCausalLM(QWenLMHeadModel):
    ...
    def forward(
        ...
        if loc_targets is not None and len(loc_targets) > 0:
            ...
            loc_positions = ((input_ids.flatten() == ref_token)
    ...
    def initialize_vision_tokenizer(
        ...
        if config.use_im_start_end:
            ...
            config.im_start_token = tokenizer.convert_tokens_to_ids('<img>')
            config.im_end_token = tokenizer.convert_tokens_to_ids('</img>')
    ...
    def generate(
        ...
        if loc_inputs is not None and len(loc_inputs) > 0:
            ...
            num = (input_ids == box_token).sum()
            loc_embeds = loc_embeds[:num]
            if num == 0:
                loc_embeds = None

        input_embeds = self.transformer.encode_input_embeds(input_ids, images, loc_embeds, inputs_embeds=None)
        ...
        if hasattr(outputs, "beam_indices"):  # beam size > 1
            loc_ids = (outputs.sequences == ref_token).nonzero()
            hidden_states = outputs.hidden_states
            beam_indices = outputs.beam_indices

            for lid in loc_ids:
                # assign to box
                outputs.sequences[lid[0], lid[1] + 1] = box_token
    ...
    def prepare_inputs_for_generation(
        ...
        if inputs_embeds is not None and past_key_values is None:
            ...
            ref_token = 151851
            if loc_ids is not None and (loc_ids == ref_token).any():
                mask = loc_ids == ref_token
