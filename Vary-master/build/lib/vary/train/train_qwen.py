...
def train():
    ...
    print('from_pretrained', model_args.model_name_or_path)
    name = 'vision_model.embeddings.class_embedding'
    print('vision_tower.' + name)
    print(vision_tower.state_dict()[name][:10])
    vision_tower_high = model.get_model().vision_tower_high
    name = 'patch_embed.proj.bias'
    print('vision_tower_high.' + name)
    ...
    if model_args.freeze_lm_model:
        ...
        if model_args.freeze_vision_tower:
            print("[WARNING] Freezing vision tower while training the LM model.")
            model.get_model().vision_tower.requires_grad_(False)
            model.get_model().vision_tower_high.requires_grad_(False)
