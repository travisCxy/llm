    ...
    def multimodal_processor(self, sources):
        for source in sources:
            ...
            for sentence in source:
                ...
                # sentence["value"] = str(sentence["value"]).replace(DEFAULT_IMAGE_TOKEN, replace_token).replace('<at> <boxes>', '<ref> <box>')
        return sources
            ...
            for conversation, target in zip(conversations, targets):
                ...
                for i, rou in enumerate(re_rounds):
                    ...
                    if len(parts) != 2:
                        break
                    parts[0] += sep
                    round_len = len(self.tokenizer(rou, add_special_tokens=False).input_ids) + len(
                        self.tokenizer(conv.sep, add_special_tokens=False).input_ids)
        ...
        if isinstance(data, dict):
            if 'image' in data:
                ...
                try:
                    ...
                    # if '<pic' in text and random.random() < 0.5:
                ...
                if '<box>' in data['conversations'][0]['value'] or '<box>' in data['conversations'][1]['value'] or \
                        '<point>' in data['conversations'][0]['value'] or '<point>' in data['conversations'][1][
                    'value']:
                    mode = 0
                else:
                    mode = 1
