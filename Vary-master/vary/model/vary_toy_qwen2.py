...
class varyQwenModel(Qwen2Model):
    config_class = varyConfig

    def __init__(self, config: Qwen2Config):
        super(varyQwenModel, self).__init__(config)

        # self.vision_tower = CLIPVisionModel.from_pretrained('/mnt/ceph/pretrained/Ucas/vit-large-patch14/')
    ...
    def initialize_vision_modules(
        ...
        # self.vision_tower = CLIPVisionModel.from_pretrained('/mnt/ceph2/pretrained/Ucas/vit-large-patch14/')
        # self.vision_tower_high = build_sam_vit_b(checkpoint='/mnt/ceph2/pretrained/Ucas/vision_tower_high.pth')
        self.vision_tower = CLIPVisionModel.from_pretrained('/cache/vit-large-patch14/')
        self.vision_tower_high = build_sam_vit_l(checkpoint='/mnt/ceph2/Vary/runs/05212/vision_tower_high.pth')

        # 224*224
        image_processor = CLIPImageProcessor.from_pretrained('/mnt/ceph2/pretrained/Ucas/vit-large-patch14/')
        # 1024*1024
            ...
            for cur_input_ids, cur_input_embeds, cur_image_features in zip(input_ids, inputs_embeds, image_features):
                ...
                if use_im_start_end:
                    ...
                    for image_start_token_pos, per_cur_image_features in zip(image_start_tokens, cur_image_features):
                        ...
                        print('cur_input_embeds[:image_start_token_pos + 1]', cur_input_embeds[:image_start_token_pos + 1].shape)
                        print('per_cur_image_features', per_cur_image_features.shape)
                        print('cur_input_embeds[image_start_token_pos + num_patches + 1:]', cur_input_embeds[image_start_token_pos + num_patches + 1:].shape)
...
class varyQwenForCausalLM(Qwen2ForCausalLM):
    config_class = varyConfig
    ...
    def initialize_vision_tokenizer(
        ...
        if config.use_im_start_end:
            ...
            self.resize_token_embeddings(len(tokenizer))
            config.im_start_token, config.im_end_token = tokenizer.convert_tokens_to_ids(
