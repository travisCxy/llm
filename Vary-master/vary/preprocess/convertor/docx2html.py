...
class DocxConvertor:
    ...
    def parse_paragraph(self, paragraph):
        ...
        if math:
            text += '\\)'
        if bool(re.match(r'^[一二三四五六七八九十]+、', text)):
            if '计算题' in text:
                self.qtype = '计算题'
            elif '连线题' in text:
                self.qtype = '连线题'
                ...
                self.qtype = ''
        elif self.qtype != '计算题' and bool(re.match(r'^\d+\.', text)):
            self.qtype = '计算题' if any(title in text for title in ['竖式计算', '直接写出得数', '用递等式计算']) else ''
        elif self.qtype == '计算题':
            if not bool(re.match(r'^\d+\.', text)):
                texts = re.split(r'\s{2,}', text)
                if len(texts) > 1:
                    if texts[0] == '':
                        texts = texts[1:]
                    if texts[-1] == '':
                ...
                if len(texts) > 1:
                    ...
                    text = '<tab/>'.join(texts)
        if self.format > 0:
            text = f'<p id="{self.id}"{prop}>{text}</p>'
            self.id += 1
    ...
    def replace_img(self, body, image, text, box, id, output_dir):
        x0, y0, x1, y1 = box
        ...
        path = os.path.join(output_dir, 'tmp')
        if not os.path.exists(path):
            os.makedirs(path)
        path = os.path.join(path, f'{id}.png')
        crop.save(path)
        w = (x1 - x0) * 768 // image.width
        h = (y1 - y0) * 768 // image.width
        path = f'{os.path.basename(output_dir)}/tmp/{id}.png'
        body = body.replace(text, f'<img src="{path}" width={w} height={h}>')
        return body

    def pretty(self, body, format=None, img_path=None, output_dir='output'):
        if format is None:
            format = self.format
        if format > 0:
            ...
            if img_path is not None:
                ...
                if len(matches) > 0:
                    pics = {}
                    with open(os.path.join(output_dir, 'ocr.txt'), 'r', encoding='utf-8') as f:
                        for line in f:
                    ...
                    for i in range(len(matches)):
                        ...
                        if id not in pics:
                            ...
                            continue
                        body = self.replace_img(body, image, matches[i][0], pics[id], id, output_dir)
                else:
                    matches = re.findall(r'(<box>\((\d+),(\d+)\),\((\d+),(\d+)\)</box>)', body)
                    image = Image.open(img_path)
                    for i in range(len(matches)):
                        match = matches[i]
                        body = self.replace_img(body, image, match[0], map(int, match[1:]), i, output_dir)
