...
from convertor.html_formatter import HtmlFormatter
...
def convert_latex_to_omml_element(text, rPr=None):
    ...
    try:
        ...
        # 左大括号
        if text.startswith('\\left\\{\\begin{matrix}') and len(omml_tree.getchildren()) == 3:
            try:
                r = omml_tree.getchildren()[0]
                t_text = r.find(f'.//{{{namespaces["m"]}}}t').text
                assert t_text == '{'
                r = omml_tree.getchildren()[2]
                t_text = r.find(f'.//{{{namespaces["m"]}}}t').text
                assert t_text is None or t_text == ''
                eqArr = omml_tree.getchildren()[1]
                assert eqArr.tag == f'{{{namespaces["m"]}}}eqArr'
                d = etree.Element(f'{{{namespaces["m"]}}}d')
                dPr = etree.Element(f'{{{namespaces["m"]}}}dPr')
                etree.SubElement(dPr, f'{{{namespaces["m"]}}}begChr',
                                 attrib={f'{{{namespaces["m"]}}}val': '{'})
                etree.SubElement(dPr, f'{{{namespaces["m"]}}}endChr',
                                 attrib={f'{{{namespaces["m"]}}}val': ''})
                d.append(dPr)
                e = etree.Element(f'{{{namespaces["m"]}}}e')
                e.append(eqArr)
                d.append(e)
                omml_element.append(d)
                return omml_element
            except:
                pass
...
class HtmlConvertor:
                ...
                if child.name == 'style':
                    ...
                    if font_size is not None:
                        ...
                        if font_size > self.font_size:
                            # print(self.path, f'font_size: {font_size} -> {self.font_size}')
    ...
    def set_paragraph_format(self, paragraph, node):
        ...
        if '□' in node.text or '○' in node.text:
            textAlignment = OxmlElement('w:textAlignment')
            textAlignment.set(f'{{{namespaces["w"]}}}val', 'center')
            pPr.append(textAlignment)
        ...
        if tab_count == 1:
            ...
            if val == 'right':
                ...
                tab.set(f'{{{namespaces["w"]}}}val', 'right')
                tab.set(f'{{{namespaces["w"]}}}pos', '8906')
                tabs.append(tab)
                pPr.append(tabs)
                tab_count = 0
        if tab_count > 0:
            ...
            for i in range(tab_count):
                ...
                tab.set(f'{{{namespaces["w"]}}}pos', str(pos * (i + 1) + left * 2))
                tabs.append(tab)
            pPr.append(tabs)
    ...
    def read_ocr(self, path):
        ...
        with open(path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                #     return False
                if 'id' in line:
                    match = re.search(r'<ref id="(\d+)">(.*?)</ref>', line)
                    if match is not None:
                        id = match.group(1)
                        text = match.group(2)
                        match = re.search(r'<box>\((\d+),(\d+)\),\((\d+),(\d+)\)</box>', line)
                        self.refs[id] = {'box': [int(match.group(1)), int(match.group(2)), int(match.group(3)), int(match.group(4))],
                                         'result': text}
                if '<pic' in line:
    ...
    def convert(self, path, html_str, output, ocr=None):
        # if '<table' in html_str:
        #     # print(output, 'table error')
        #     return
        self.image = Image.open(path)
        self.image_id = 0
        ...
        self.line_spacing = 0
        if ocr is not None:
            if not self.read_ocr(ocr):
                print(output, 'table error')
                return
        formatter = HtmlFormatter(self.refs, self.pics)
        self.soup = formatter.pretty(html_str)
        table = self.soup.find('table')
        if table is not None:
            print(output, 'table error')
            return
        self.line_height = formatter.line_height
        self.line_spacing = formatter.line_spacing
        if not self.check_pic(self.soup):
            # page_count = self.page_count(output.replace('-05242', ''))
            print(output, 'pic error')
            return

        self.save_font(output)
    ...
    def one_page(self, path):
        ...
        if self.font_size > 12 or self.font_family in ['微软雅黑', 'MicroftJhengHei', '华文细黑', '华文中宋',
            ...
            for pt in range(self.spacing_line-24, 480-24, -24):
                ...
                page_count = self.page_count(tmp_path)
                if page_count < 2:
