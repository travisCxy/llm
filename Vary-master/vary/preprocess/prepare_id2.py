    ...
    for path in files:
        ...
        try:
            ...
            if text3 is None:
                ...
                continue
            body = re.match(r'.*<body>(.*)</body>.*', text3, re.DOTALL).group(1)
            if body.startswith('<table') and body.endswith('</table>') and body.count('<table') == 1:
                ...
                if len(regions) > 0:
                    try:
                        text = match_regions(text3, regions)
                        if text is not None:
                            text3 = text
                            match = True
                    except Exception as e:
                ...
                if match:
                    ...
                    progress_queue.put(('error', doc_path))
                    continue
                if ocr.count('□') > 2 and '□' not in text3:
                    progress_queue.put(('□', path))
                    continue
                if match:
                    progress_queue.put(('match', path))
                matches = re.findall(r'(<p id="([^"]*)")', text3)
                for match in matches:
                    if ';' in match[1]:
                        continue
                    id = int(match[1])
                    if id >= 1000:
                        text3 = text3.replace(match[0], f'<p')
...
def prepare_tiku(tokenizer, parallel):
    ...
    if parallel:
        process_parallel(process, files, tokenizer, True, True, progress_queue, '/mnt/ceph2/datasets/tiku/vary/conversations_tiku_id2.json')
    else:
...
def prepare_sjb(tokenizer, parallel):
    ...
    progress_queue = multiprocessing.Queue()
    if parallel:
        process_parallel(process, files, tokenizer, True, False, progress_queue, '/mnt/ceph2/datasets/tiku/vary/conversations_sjb_id2.json')
    else:
...
def prepare_camera(tokenizer, parallel):
    ...
    progress_queue = multiprocessing.Queue()
    if parallel:
        process_parallel(process_camera, files, tokenizer, True, True, progress_queue, '/mnt/ceph2/datasets/tiku/vary/conversations_camera_id2.json')
    else:
...
def check():
    from convertor.docx2html import DocxConvertor
    docx = DocxConvertor()
    conversations = json.load(open('/mnt/ceph2/datasets/tiku/vary/conversations_camera_id2.json', encoding='utf-8'))
    # count = 0
