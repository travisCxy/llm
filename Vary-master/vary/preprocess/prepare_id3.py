...
def process(files, tokenizer, footer, with_font, progress_queue):
    ...
    for path in files:
        ...
        try:
            ...
            text2 = match_pics(text2, pics, box=True)
            if text2 is None:
...
def prepare_tiku(tokenizer, parallel):
    ...
    if parallel:
        process_parallel(process, files, tokenizer, True, True, progress_queue, '/mnt/ceph2/datasets/tiku/vary/conversations_tiku_id3.json')
    else:
...
def check():
    from convertor.docx2html import DocxConvertor
    docx = DocxConvertor()
    conversations = json.load(open('/mnt/ceph2/datasets/tiku/vary/conversations_tiku_id3.json', encoding='utf-8'))
    # count = 0
